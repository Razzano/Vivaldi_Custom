...
try {
  ...
  $Path = "$dstdir\move-active-tab.js"
  if (Test-Path $Path) {del $Path}
  write-host "Deleting all files from style\move-active-tab folder"
  del $dstdir\style\move-active-tab\*.css
  del $dstdir\style\move-active-tab\*.png
  write-host "Deletes Done"
  write-host "Writing browser.html back to prior move-active-tab install"
  findstr /v /r "move-active-tab\.[c-j]s" $dstdir\browser.html > $srcdir\restore.html
