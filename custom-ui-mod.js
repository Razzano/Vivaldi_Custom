...
*/
  ...
  function keyCombo(id, combination) {
	const SHORTCUTS = {Esc: () => removeDialog(Array.from(webviews.keys()).pop())}
  ...
  function dialogTab(linkUrl) {
    let browser = $q("#browser"),
	    ...
	    divContainer = $c("div", {id: "dialogContainer", className: "dialog-tab"}, [{type: "click", fn: function (event) {if (event.target === this) removeDialog(webviewId)}}]),
        webview = $c("webview", {src: linkUrl}),
        divOptionContainer = $c("div", {id: "dialogOptionContainer", innerHTML: getEllipsisContent()}),
        progressBarContainer = $c("div", {id: "progressBarContainer"}),
        progressBar = $c("div", {id: "progressBar"}),
		timeout;
	if (browser.hasAttribute("dialog-tab")) browser.removeAttribute("dialog-tab");
    ...
    webview.addEventListener("loadstop", () => document.getElementById("progressBar").style.display = "none");
    divOptionContainer.addEventListener("mouseover", () => {
	  browser.setAttribute("dialog-tab", true);
	...
	divOptionContainer.addEventListener("mouseleave", () => {
	  browser.removeAttribute("dialog-tab");
      if (!timeout) {
        timeout = setTimeout(() => {
		  divOptionContainer.innerHTML = "";
          setTimeout(() => divOptionContainer.innerHTML = getEllipsisContent(), 20);
        }, 20);
	  }
    });
  ...
  function showWebviewOptions(webviewId, thisElement) {
    ...
    if (webview && document.getElementById(inputId) === null) {
      let webviewSrc = webview.src,
          input = $c("input", {id: "inputLink", value: webviewSrc, type: "text"}),
          buttonBack = $c("button", {id: "buttonBack", title: "Go Back"}, [{type: "click", fn: function (event) {if (event.target === this || this.firstChild) webview.back()}}]),
          buttonForward = $c("button", {id: "buttonForward", title: "Go Forward"}, [{type: "click", fn: function (event) {if (event.target === this || this.firstChild) webview.forward()}}]),
          buttonNewTab = $c("button", {id: "newTabButton", innerHTML: getNewtabContent()}, [{type: "click", fn: function (event) {if (event.target === this || this.firstChild) { openNewTab(inputId, true)}}}]),
          buttonBackgroundTab = $c("button", {id: "backgroundButton", innerHTML: getBacktabContent()}, [{type: "click", fn: function (event) {if (event.target === this || this.firstChild) {openNewTab(inputId, false)}}}]),
		  buttonClose = $c("button", {id: "buttonClose", innerHTML: getCloseButtonContent()}, [{type: "click", fn: () => {removeDialog(webviewId)}}]);
  ...
  function getCloseButtonContent() {
    return '<span id="close-button" title="Close Dialog"><svg xmlns="http://www.w3.org/2000/svg" style="'+ dialogcloseIcon +'"/></span>';
  }
