(() => {
  ...
  const initializeDelay = 200,
		...
		pos4Tooltip = "Position Menu > CENTER",
  ...
  function setOptionsMenu() {
	...
	try {
      ...
      let ip = $q('div#options-menu input[type="checkbox"]', true);
      for (let i = 0; i < ip.length; i++) ip[i].onclick = e => onOptionsMenuInput(e.target.id);
	  let rd = $q('div#options-menu input[type="radio"]', true);
