...
/*
  function dialogWindow(linkUrl) {
    let webview = document.createElement("webview"),
        divContainer = document.createElement("div");
	webview.id = "linkDialogWindow";
    webview.setAttribute("src", linkUrl);
	webview.style = "background-color: rgba(23,25,26); border: 2px solid #3f81ea; border-radius: 10px; height: 95%; margin: auto; overflow: hidden; padding: 0; width: 95%;";
    webview.addEventListener("loadstart", function () {
      this.style.backgroundColor = "rgba(23,25,26,.7)";
	  this.style.borderColor = "rgba(59,129,230)";
    });
    divContainer.setAttribute("class", "dialog-window");
	divContainer.style = "background-color: rgba(23,24,25,.4); bottom: 0; left: 0; position: fixed; right: 0; top: 0; z-index: 9999999";
...
/*
  function setContextMenu() {
	try {
      chrome.contextMenus.create({'id': 'dialog-window-link', 'title': 'Open Link in Dialog Window', 'contexts': ['link'] });
      chrome.contextMenus.onClicked.addListener(function (itemInfo) {
        chrome.windows.getLastFocused(function (window) {
          if (window.id === vivaldiWindowId && window.state !== 'minimized') { if (itemInfo.menuItemId === "dialog-window-link") dialogWindow(itemInfo.linkUrl); }
        });
      });
	} catch(ex) {}
  }
...
/*   https://forum.vivaldi.net/topic/38084/open-link-or-new-tab-in-a-dialog-mod   */
    ...
    function setUrlClickObserver() {
        ...
        document.addEventListener("mousedown", function (event) {
            if (event.ctrlKey && event.altKey && (event.button === 0 || event.button === 1)) showDialog(event);
            if (event.button === 1) timer = setTimeout(() => showDialog(event), 500);
        });
    ...
    function createContextMenuOption() {
        ...
        chrome.contextMenus.create({ id: "search-dialog-tab", title: '[Dialog Tab] Search for "%s"', contexts: ["selection"] });
        chrome.contextMenus.create({ id: "select-search-dialog-tab", title: "[Dialog Tab] Search with", contexts: ["selection"] });
        chrome.contextMenus.onClicked.addListener(function (itemInfo) {
            chrome.windows.getLastFocused(function (window) {
                if (window.id === vivaldiWindowId && window.state !== "minimized") {
                    if (itemInfo.menuItemId === "dialog-tab-link") {
                        ...
                        let engineId = itemInfo.menuItemId.substr(
    ...
    function createContextMenuSelectSearch() {
        searchEngineCollection.forEach(function (engine) {
            if (!createdContextMenuIds.includes(engine.id)) {
                chrome.contextMenus.create({ id: "select-search-dialog-tab" + engine.id, parentId: "select-search-dialog-tab", title: engine.name, contexts: ["selection"] });
    ...
    function keyCombo(id, combination) {
        const SHORTCUTS = {
            "Ctrl+Alt+Period": () => {
                chrome.tabs.query({ active: true }).then((tabs) => vivaldi.utilities.getSelectedText(tabs[0].id, (text) => dialogTabSearch(defaultSearchId, text)))
    ...
    function dialogTab(linkUrl) {
		...
		webview.style.borderRadius = "0 0 10px 10px";
		webview.style.height = "95%";
        webview.style.margin = "auto";
        webview.style.overflow = "hidden";
        webview.style.width = "95%";
        webview.addEventListener("loadstart", function () {
            document.getElementById("progressBar-" + webviewId).style.display = "block";
            if (document.getElementById("input-" + this.id) !== null) document.getElementById("input-" + this.id).value = this.src;
        });
        webview.addEventListener("loadstop", function () { document.getElementById("progressBar-" + webviewId).style.display = "none" });
		divOptionContainer.style.alignItems = "center";
		divOptionContainer.style.backgroundColor = "#000";
		divOptionContainer.style.borderRadius = "10px 10px 0 0";
		divOptionContainer.style.color = "white";
		divOptionContainer.style.height = "27px";
		divOptionContainer.style.margin = "auto";
		divOptionContainer.style.padding = "0";
		divOptionContainer.style.position = "relative";
		divOptionContainer.style.textAlign = "center";
        divOptionContainer.style.top = "0";
		divOptionContainer.style.width = "95%";
        divOptionContainer.style.zIndex = "8";
        ...
        divContainer.style.backdropFilter = "var(--backgroundBlur)";
		divContainer.style.backgroundColor = "var(--colorBgAlphaHeavy)";
		divContainer.style.boxShadow = "0 0 0 1px rgba(0, 0, 0, 0.1), 0 3px 10px rgba(0, 0, 0, 0.3)";
    ...
    function showWebviewOptions(webviewId, thisElement) {
        ...
        if (webview && document.getElementById(inputId) === null) {
            ...
            input.style.background = "var(--colorAccentBgAlpha)";
			input.style.border = "unset";
            input.style.color = "white";
			input.style.height = "22px";
            input.style.margin = "0";
            input.style.padding = "0 0 2px 6px";
    ...
    function setBackButtonContent(buttonBack) {
        ...
        if (svg) buttonBack.appendChild(svg.cloneNode(true));
        else buttonBack.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.2 288 416 288c17.7 0 32-14.3 32-32s-14.3-32-32-32l-306.7 0L214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z"/></svg>';
    }

    function setForwardButtonContent(forwardButton) {
        let svg = document.querySelector('.button-toolbar [name="Forward"] svg');
        if (svg) forwardButton.appendChild(svg.cloneNode(true));
        else forwardButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512"><path d="M438.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-160-160c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L338.8 224 32 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l306.7 0L233.4 393.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0l160-160z"/></svg>';
    }
