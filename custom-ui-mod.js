(() => {
  ...
  function getCurrentTabUpdated() {
    favTimer = setInterval(() => { getCurrentTab(); resetUIElements() }, favInterval);
  }
  ...
  function optionsMenu() {
	...
	try {
      divX.innerHTML = '\
        <button id="options-button" class="ToolbarButton-Button custom-button optionsButton" title="'+ optionsMenuTooltip +'" type="button" tabindex="-1">\
          <span>\
  ...
  function resetUIElements() {
    let divOpt = $q('footer > div.toolbar-statusbar > div#options'),
	    spanCal = $q('footer span#calendar');
	try {
	  if (!divOpt) optionsMenu();
	  if (!spanCal) getCalendarHolder();
	} catch(ex) {}
  }
  ...
  window.addEventListener("dragend", e => setTimeout(() => { e.preventDefault(); homeToRestart(homeRestart) }, resizeDelay));
