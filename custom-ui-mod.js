(() => {
  ...
  const initializeDelay = 200,
		...
		restartLTIcon = 'background: url(/style/icons/restartLT.png) center no-repeat',
  ...
  function initialize() {
	...
	chrome.storage.local.get(['rewindForwardKey'], result => {
      ...
      setTimeout(function() { if (rewindForward) browser.setAttribute('rewind-forward', true);
      else browser.removeAttribute('rewind-forward')}, initializeDelay)
    });
  ...
  function getCurrentTab() {
    ...
    try {
      ...
      chrome.tabs.query({currentWindow: true, active: true}, tabs => {
		let browser = $q('#browser');
		if (browser.className.match(/Vivaldi(1|5|7|8)/)) img.src = '/style/icons/settingsLT.png';
        else if (tabs[0].title.match(/Extensions/gi)) img.src = '/style/icons/extensions.png';
        ...
        else if (tabs[0].title.match(/Tampermonkey/g)) img.src = '/style/icons/tampermonkey16.png';
		else if (tabs[0].title.match(/True Key/gi)) iimg.src = '/style/icons/truekey.png';
  ...
  function homeToRestart(e) {
    ...
    try {
      if (e) {
        ...
        if (browser.hasAttribute("theme-light")) homeBtnImg.style = restartLTIcon;
		else homeBtnImg.style = restartIcon;
  ...
  function onClose() {
	...
	window.removeEventListener("drop", e => setTimeout(() => { e.preventDefault(); homeToRestart(homeRestart) }, resizeDelay));
	window.removeEventListener("fullscreenchange", () => setTimeout(() => onResize(), resizeDelay));
  ...
  function optionsMenu() {
    ...
    divX.innerHTML = '\
      <button id="options-button" class="ToolbarButton-Button custom-button" title="'+ optionsMenuTooltip +'" type="button" tabindex="-1">\
        <span>\
		  <svg xmlns="http://www.w3.org/2000/svg" style="'+ optionsIcon +'"/>\
		</span>\
      </button>\
    </div>';
    divY.innerHTML = '\
      <div id="div0">\
        ...
        <button id="options-menu-close" class="button" style="'+ closeIcon +'" title="'+ optionsCloseTooltip +'"></button>\
      </div>\
  ...
  function setToggleButton() {
    let tabs = $q('#tabs-container'),
		...
		toggle = $c('button', {id: 'toggle-toolbars', className: 'ToolbarButton-Button custom-button toggle-toolbars', title: toggleTooltip}, [{type: 'click', fn: function () {setToolbars(); if ($q('div#options-menu')) onOptionsMenuPosition(positionOptionsMenu)}}]);
  ...
  window.addEventListener("drop", e => setTimeout(() => { e.preventDefault(); homeToRestart(homeRestart) }, resizeDelay));
  window.addEventListener("fullscreenchange", () => setTimeout(() => onResize(), resizeDelay));
