...
*/
  ...
  function dialogTab(linkUrl) {
    let divContainer = $c("div"),
        webview = $c("webview"),
	    webviewId = "dialogWebView",
        divOptionContainer = $c("div"),
        progressBarContainer = $c("div"),
        progressBar = $c("div"),
		timeout;
	...
	divOptionContainer.addEventListener("mouseleave", () => {
      if (!timeout) {
        timeout = setTimeout(() => {
		  divOptionContainer.innerHTML = "";
          setTimeout(() => divOptionContainer.innerHTML = getEllipsisContent(), 100);
        }, 20);
	  }
    });
  ...
  function showWebviewOptions(webviewId, thisElement) {
    ...
    if (webview && document.getElementById(inputId) === null) {
      let webviewSrc = webview.src,
          input = $c("input", "text"),
          buttonBack = $c("button"),
          buttonForward = $c("button"),
          buttonNewTab = $c("button"),
          buttonBackgroundTab = $c("button"),
		  buttonClose = $c("button");
	  input.id = "inputLink";
	  ...
	  buttonClose.addEventListener("click", () => { removeDialog(webviewId) });
