(() => {
  ...
  function optionsMenu() {
    ...
    divY.innerHTML = '\
      <div id="div0">\
		...
		<span id="spanImg">\
		  <span><svg xmlns="http://www.w3.org/2000/svg" style="'+ optionsOnIcon +'"/></span>\
  ...
  window.onload = () => setTimeout(() => initialize(), initializeDelay);
  window.onresize = () => setTimeout(() => onResize(), resizeDelay);
  window.ondrop = e => { e.preventDefault(); homeToRestart(homeRestart) };
  window.onunload = () => {
	chrome.storage.local.set({calendarFormatKey: calendarFormat.toString()});
    chrome.storage.local.set({closeButtonKey: closeButton});
	chrome.storage.local.set({customCssKey: customCss});
