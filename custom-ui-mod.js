(() => {
  ...
  function initialize() {
	...
	try {
	  chrome.storage.local.get(['calendarFormatKey'], result => {
		...
		browser.setAttribute('calendar-format', calendarFormat);
      });
  ...
  function setCalendarHolder() {
    let browser = $q('#browser'),
	    cal = $c('span', {id: 'calendar', className: 'aCal', title: calendarTooltip}, [{type: 'mouseover', fn: function () {getCalendarText()}}]),
        clk = $q('.ClockButton');
	try {
	  if (!calendarFormat) calendarFormat = '1';
	  cal.textContent = aCalendar(calendarFormat);
